...
export default async function OwnerOrderDetailPage({
  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        <Link href="/owner/orders" className="text-sm font-medium text-violet-700 hover:text-violet-800">
          ← Back to Orders
        </Link>
        ...
        <p className="text-sm text-slate-500">
          {order.contactEmail} · Placed {new Date(order.createdAt).toLocaleString()}
        </p>
      </div>

      <div className="grid gap-6 sm:grid-cols-2">
        <OrderStatusForm orderId={order.id} currentStatus={order.status} />
        <TrackingForm orderId={order.id} trackingNumber={order.trackingNumber} carrier={order.carrier} />
      </div>
